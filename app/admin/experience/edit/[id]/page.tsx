...
export default function EditExperience({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setError(d.error || 'Failed to update experience record.')
      }
    } catch {
  ...
  if (loading) {
    return (
      <div className="bg-pokedex-screen-dark border-4 border-black rounded-xl shadow-[4px_4px_0_#000] p-8 text-center scanlines">
        <span className="font-press-start text-pokedex-screen text-xs blink">▶ LOADING EXPERIENCE DATA...</span>
      </div>
    )
  }

  if (!formData.company && !loading) {
    return (
      <div className="bg-pokedex-dark border-4 border-black rounded-xl shadow-[4px_4px_0_#000] p-10 text-center">
        ...
        <Link href="/admin/experience" className="retro-btn bg-pokedex-dark text-pokedex-gray border-pokedex-gray px-5 py-2 font-press-start text-[9px]">
          ◄ BACK TO EXPERIENCE
        </Link>
      </div>
    )
    ...
    <div className="max-w-2xl mx-auto space-y-5">
      ...
      <div className="bg-pokedex-dark rounded-xl border-4 border-black shadow-[4px_4px_0_#000] overflow-hidden">
        <div className="flex items-center gap-4 px-5 py-3 bg-pokedex-red border-b-4 border-black">
          ...
          <span className="font-press-start text-white text-[10px] tracking-widest">EDIT EXPERIENCE RECORD</span>
        </div>
        <div className="px-5 py-3 flex items-center justify-between">
          <p className="font-vt323 text-pokedex-light text-xl opacity-70">Modify work experience entry</p>
